import { rankHand, compareRankedHands } from "../rules/ranking.js";
...
export function compareHands(
  ...
  return compareRankedHands(rankHand(a.cards), rankHand(b.cards), variant);
}
...
export function getAIPersonalityParams(
...
export function makeAIDecision(context: AIDecisionContext): {
  action: AIAction;
  shouldRaise: boolean;
} {
  ...
  if (
    context.playersRemaining === 2 &&
    !context.isBlind &&
    context.roundNumber >= 3
  ) {
    const showChance = Math.min(
      0.8,
      (context.roundNumber - 2) * 0.15 + context.handStrength * 0.3,
    );
    if (rand < showChance) {
  ...
  if (context.handStrength < params.foldThreshold) {
    ...
    const foldChance =
      (1 - context.handStrength) *
      (1 + context.potOdds * 0.5) *
      (context.playersRemaining / 4);
  ...
  if (context.handStrength > params.raiseThreshold) {
    ...
    if (rand < params.slowPlayChance) {
      ...
      return {
        action: context.isBlind ? "blind" : "chaal",
        shouldRaise: false,
      };
    }
    return { action: "raise", shouldRaise: true };
  ...
  const raiseChance =
    ((context.handStrength - params.foldThreshold) /
