...
export default function LobbyScreen() {
  ...
  const { roomSnapshot, leaveGame } = useGameStore();
  const [loadError, setLoadError] = useState<string | null>(null);
  const me = roomSnapshot?.players.find((p) => p.odic === user?.id);
  ...
  const loadRooms = useCallback(async () => {
    ...
    try {
      ...
      setLoadError("Could not load tables. Pull down to retry.");
    } finally {
      setIsLoading(false);
    }
  }, []);
  ...
  const createFriendTable = async () => {
    try {
      ...
      if (!result.success || !result.room?.roomCode)
      ...
      Alert.alert(
        "Your table is ready",
        `Room code: ${result.room.roomCode}\n\nThe game starts when everyone marks ready.`,
        [
          { text: "Keep waiting", style: "cancel" },
          {
            text: "Share invite",
            onPress: () => {
              void Share.share({ title: "Join my table", message }).then(
                (shareResult) => {
                  if (shareResult.action === Share.sharedAction) {
                    void api
                      .post("/referrals/share", {
                        platform: "NATIVE",
                        campaign: "friend_table",
                      })
                      .catch(() => {});
                  }
                },
  ...
  const joinFriendTable = async () => {
    ...
    if (!/^[A-Z0-9]{6}$/.test(code)) {
      Alert.alert(
        "Check the code",
        "Enter the 6-character room code your friend sent.",
      );
      return;
    }
    setJoining(true);
    ...
    <View style={styles.container}>
      ...
      <Animated.View entering={FadeInDown.delay(100)} style={styles.header}>
        <View style={styles.profileRow}>
          <View
            style={[
              styles.avatar,
              user?.equippedItems?.AVATAR_FRAME === "saffron-frame" && {
                borderColor: "#f5a524",
                borderWidth: 3,
              },
            ]}
          ...
          <View>
            ...
            <AnimatedChipCount
              ...
              style={styles.chips}
            />
            <Text style={styles.beliMini}>
              {user?.beliBalance || 0} Club Points
            </Text>
          </View>
        </View>
      </Animated.View>
      ...
      {roomSnapshot ? (
        <View style={styles.friendActions}>
          <Text style={styles.username}>
            {roomSnapshot.name} · {roomSnapshot.roomCode}
          </Text>
          <Text style={styles.roomInfo}>
            {roomSnapshot.variant} · Boot {roomSnapshot.bootAmount} · Seeing
            cards doubles your bet
          </Text>
          {roomSnapshot.players.map((p) => (
            <Text key={p.odic} style={{ color: "#fff" }}>
              {p.username}
              {p.odic === user?.id ? " (you)" : ""} ·{" "}
              {!p.connected
                ? "Reconnecting"
          ...
          <Text style={{ color: "#fff" }}>
            {roomSnapshot.status === "playing"
              ? "Your seat is saved for the next hand."
              : "The hand starts when all seated players are ready."}
          </Text>
          <PressableButton
            disabled={joining}
            onPress={async () => {
              setJoining(true);
              try {
                const result = await gameSocket.ready(!me?.isReady);
          ...
          <PressableButton variant="secondary" onPress={() => void leaveGame()}>
            <Text style={styles.createText}>{t("table_leave")}</Text>
          </PressableButton>
          <PressableButton
            variant="secondary"
            onPress={() => {
              if (roomSnapshot.roomCode)
                void Share.share({
                  message: `Join my Teen Patti table: ${roomSnapshot.roomCode}`,
                });
            }}
          >
            <Text style={styles.createText}>Share room code</Text>
          </PressableButton>
        </View>
      ) : (
        <>
          <View style={styles.friendActions}>
            <Pressable
              onPress={() => void createFriendTable()}
              style={styles.createButton}
            >
              <Text style={styles.createText}>Create friend table</Text>
            </Pressable>
          ...
          >
            <Pressable
              onPress={() => router.push("/(main)/referrals")}
            ...
            >
              <View style={styles.inviteCopy}>
                ...
                <Text style={styles.inviteTitle}>
                  Both earn 100 Club Points
                </Text>
                <Text style={styles.inviteSub}>
                  After your friend's first real multiplayer game
                </Text>
              </View>
            ...
            >
              ...
              <View>
                ...
                <Text style={styles.quickPlaySub}>
                  Play instantly with AI opponents
                </Text>
              </View>
            </PressableButton>
          </Animated.View>
              ...
              <Pressable onPress={() => void joinPublicRoom(room)}>
                <GlassCard style={styles.roomCard}>
                  <View style={styles.roomHeader}>
                    ...
                    <View
                      style={[
                        ...
                        {
                          backgroundColor:
                            VARIANT_COLORS[room.variant] || "#666",
                        },
                      ]}
                    >
                  ...
                  <Text style={styles.roomInfo}>
                    ...
                    chips • Boot: {formatChips(room.minBet)}
                  </Text>
                  <View style={styles.roomFooter}>
                    <View style={styles.playerDots}>
                      {Array.from({ length: room.currentPlayers }).map(
                        (_, i) => (
                          <View
                            key={i}
                            style={[
