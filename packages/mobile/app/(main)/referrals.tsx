...
export default function ReferralsScreen() {
  ...
  const recordShare = async (platform: "NATIVE" | "COPY") => {
    try {
      await api.post("/referrals/share", {
        platform,
        campaign: "table_circle",
      });
    } catch {
  ...
  const equip = async (itemId: string, type: string) => {
    setBusyItem(itemId);
    try {
      const result = await api.post<{ equippedItems: Record<string, string> }>(
        "/referrals/equip",
        { itemId, equip: summary?.equippedItems[type] !== itemId },
      );
      updateUser(result);
      await load();
    } catch (e) {
  ...
  const redeem = (itemId: string, name: string, costBeli: number) => {
    Alert.alert(
      "Unlock this extra?",
      `${name} costs ${costBeli} Club Points.`,
      [
        { text: "Not now", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            setError("");
            try {
              const result = await api.post<{ beliBalance: number }>(
                "/referrals/redeem",
                { itemId },
              );
  ...
  if (!summary && !error) {
    return (
      <View style={styles.center}>
        <ActivityIndicator color="#FFD66B" size="large" />
      </View>
    );
  }
    ...
    <View style={styles.container}>
      ...
      >
        ...
        <Text style={styles.intro}>
          Your friend completes one real multiplayer game, then both of you earn{" "}
          {summary.activationRewardBeli} Club Points. Signups and bot games do
          not count.
        </Text>
        ...
        <View style={styles.hero}>
          <View
            ...
            accessibilityLabel="You invite, your friend plays, both earn"
          >
            <View style={styles.flowStep}>
              <Text style={styles.flowIcon}>🤝</Text>
              <Text style={styles.flowText}>You invite</Text>
            </View>
            <Text style={styles.thread}>——</Text>
            <View style={styles.flowStep}>
              <Text style={styles.flowIcon}>🃏</Text>
              <Text style={styles.flowText}>They play</Text>
            </View>
            <Text style={styles.thread}>——</Text>
            <View style={styles.flowStep}>
              <Text style={styles.flowIcon}>✨</Text>
              <Text style={styles.flowText}>Both earn</Text>
            </View>
          </View>
          <View style={styles.statsRow}>
            <View>
              <Text style={styles.statLabel}>Your Club Points</Text>
              <Text style={styles.beli}>
                {summary.beliBalance.toLocaleString("en-IN")}
              </Text>
            </View>
            <View style={styles.statRight}>
              <Text style={styles.statLabel}>Friends activated</Text>
              <Text style={styles.statValue}>{summary.stats.activated}</Text>
            </View>
          </View>
        </View>
        ...
        <View style={styles.card}>
          <View style={styles.sectionRow}>
            <View>
              <Text style={styles.eyebrow}>CLUB POINTS TRAIL</Text>
              <Text style={styles.cardTitle}>Keep the circle moving</Text>
            </View>
            <Text style={styles.cardSub}>
              {summary.stats.activated}/{nextCount}
            </Text>
          </View>
          ...
          {summary.milestones.map((milestone) => (
            <View key={milestone.count} style={styles.milestone}>
              <Text style={milestone.achieved ? styles.doneDot : styles.dot}>
                ●
              </Text>
          ...
          return (
            <View key={item.id} style={styles.rewardCard}>
              ...
              <Pressable
                onPress={() =>
                  item.owned
                    ? void equip(item.id, item.type)
                    : redeem(item.id, item.name, item.costBeli)
                }
                disabled={(!item.owned && !canAfford) || busyItem !== null}
                style={[
                  styles.redeemButton,
                  (item.owned || !canAfford) && styles.disabled,
                ]}
              >
                {busyItem === item.id ? (
        ...
        <Text style={styles.disclaimer}>
          Club Points cannot be bought, transferred, wagered, cashed out, or
          exchanged for play chips.
        </Text>
      </ScrollView>
    </View>
  );
...
const styles = StyleSheet.create({
  ...
  errorBanner: {
    color: "#fecaca",
    backgroundColor: "rgba(217,72,65,0.15)",
    borderRadius: 12,
    padding: 12,
    marginBottom: 12,
  },
