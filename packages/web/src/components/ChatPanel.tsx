...
export function ChatPanel({ onClose }: ChatPanelProps) {
  ...
  const handleSend = (text?: string) => {
    ...
    if (!socketService.sendChatMessage(currentRoom.id, message)) {
      ...
      setSendError(
        "Reconnecting—your message is still here. Try again in a moment.",
      );
      return;
    }
    setSendError(null);
  ...
  return (
    <>
      ...
      >
        <header className="flex items-center justify-between border-b border-white/10 px-4 py-3.5">
          <div className="flex min-w-0 items-center gap-2.5">
            ...
            <div className="min-w-0">
              <h2
                id={titleId}
                className="font-display text-lg font-bold text-[#F6ECD8]"
              >
                Table chat
              </h2>
              <p className="text-[10px] uppercase tracking-[0.15em] text-[#7E8D85]">
                {chatMessages.length}{" "}
          ...
          <button
            ...
            className="grid h-10 w-10 place-items-center rounded-full border border-white/10 text-[#A9B9B0] hover:bg-white/[0.06] hover:text-[#F6ECD8] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E8B04A]"
          >
        ...
        <div
          className="scrollbar-hide flex gap-2 overflow-x-auto border-b border-white/[0.06] px-4 py-2.5"
        ...
        >
          {chatMessages.length === 0 ? (
            <div className="flex h-full flex-col items-center justify-center text-center text-[#7E8D85]">
              <MessageCircle
                aria-hidden="true"
                className="mb-3 h-8 w-8 text-[#E8B04A]/55"
              />
              <p className="font-display text-lg font-bold text-[#C9D3CE]">
                The table is quiet.
              </p>
              <p className="mt-1 text-xs">
                Wish everyone luck or send the first reaction.
              </p>
            </div>
          ) : (
            chatMessages.map((message, index) => {
              ...
              return (
                <motion.article
                  ...
                  transition={{
                    delay: reducedMotion ? 0 : Math.min(index * 0.025, 0.2),
                  }}
                ...
                >
                  {message.avatarUrl ? (
                    ...
                    <div
                      aria-hidden="true"
                      className="grid h-8 w-8 shrink-0 place-items-center rounded-full border border-[#E8B04A]/25 bg-[#2A1714] text-xs font-bold text-[#E8B04A]"
                    >
                  ...
                  >
                    <span className="mb-1 px-1 text-[10px] text-[#7E8D85]">
                    ...
                    <p
                      className={cn(
                        ...
                        isMe
                          ? "rounded-tr-sm bg-[#B74035] text-[#FFF9ED]"
                          : "rounded-tl-sm bg-white/[0.075] text-[#E2E9E5]",
                      )}
                    >
                      {message.message}
                    </p>
                    <time
                      className="mt-1 px-1 text-[9px] text-[#66736D]"
        ...
        <AnimatePresence>
          {showEmojis && (
            ...
            >
              <div
                className="grid grid-cols-8 gap-1 bg-[#07110E]/60 p-3"
                aria-label="Quick reactions"
              >
                {(user?.equippedItems?.EMOTE === "dhol-reaction"
                  ? [...quickEmojis, "🥁"]
                  : quickEmojis
                ).map((emoji) => (
                  <button
                    ...
                    className="grid aspect-square place-items-center rounded-lg text-xl hover:bg-white/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#E8B04A]"
                  >
                    {emoji}
                  </button>
                ))}
        ...
        {sendError && (
          <p
            role="alert"
            className="border-t border-[#B74035]/25 bg-[#2A1714] px-4 py-2 text-xs text-[#F2B1A9]"
          >
            {sendError}
          </p>
        )}
        ...
        <form
          ...
          className="flex items-center gap-2 border-t border-white/10 p-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] sm:pb-3"
        >
          <button
            type="button"
            onClick={() => setShowEmojis((visible) => !visible)}
            aria-label={
              showEmojis ? "Hide quick reactions" : "Show quick reactions"
            }
            ...
            className={cn(
              ...
              showEmojis
                ? "border-[#E8B04A]/45 bg-[#2A1714] text-[#E8B04A]"
                : "border-white/10 bg-white/[0.05] text-[#A9B9B0]",
            )}
          >
