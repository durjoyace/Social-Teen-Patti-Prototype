...
export function BettingControls({
  ...
  const minimumRaise = baseBetAmount * 2;
  ...
  const handleActionSelect = useCallback(
    (action: ActionType) => {
      ...
      if (action === "raise") {
        ...
        setCustomBet(
          Math.min(betCeiling, Math.max(minimumRaise, baseBetAmount * 2)),
        );
  ...
  const adjustBet = useCallback(
    (multiplier: number) => {
      setCustomBet(
        Math.min(
          Math.max(baseBetAmount * multiplier, minimumRaise),
          betCeiling,
        ),
      );
      onButtonPress();
  ...
  const incrementBet = useCallback(
    (amount: number) => {
      setCustomBet((previous) =>
        Math.min(Math.max(previous + amount, minimumRaise), betCeiling),
      );
      onButtonPress();
    },
    ...
    >
      <div className="mx-auto max-w-2xl px-3 pb-[calc(env(safe-area-inset-bottom)+0.75rem)] pt-3 sm:px-5">
        <div className="mb-3 flex items-center justify-between rounded-xl border border-white/10 bg-white/[0.035] px-3 py-2 text-sm">
          <p className="flex items-center gap-2">
            ...
            <AnimatedChipCount
              value={pot}
              prefix="◉ "
              className="font-bold text-[#E8B04A]"
            />
          </p>
          <p className="flex items-center gap-2">
            ...
            <AnimatedChipCount
              value={playerChips}
              prefix="◉ "
              className="font-semibold text-[#F6ECD8]"
            />
          </p>
        </div>
        ...
        <SlideUpSheet
          ...
          onClose={closeRaiseSheet}
          title="Set your raise"
          snapPoints={[0.48, 0.68]}
        >
          <div className="space-y-5">
            <div className="flex items-center justify-center gap-5">
              ...
              <div className="min-w-36 text-center">
                <AnimatedChipCount
                  value={customBet}
                  prefix="◉ "
                  className="text-3xl font-bold text-[#E8B04A]"
                  duration={200}
                />
                <p className="mt-1 text-xs uppercase tracking-[0.16em] text-[#A8B5AF]">
                  Raise amount
                </p>
              </div>
            ...
            <div
              ...
              aria-label="Quick raise amounts"
            >
              {QUICK_BET_MULTIPLIERS.map((multiplier) => (
                <PressableButton
                  ...
                  variant={
                    customBet ===
                    Math.min(baseBetAmount * multiplier, betCeiling)
                      ? "primary"
                      : "ghost"
                  }
        ...
        {!showBetSlider && secondaryActions.length > 0 && (
          <div
            className="mb-2 grid grid-cols-2 gap-2 sm:flex"
          ...
          <div className="flex gap-2">
            {primaryActions.map((action, index) => {
              ...
              const betAmount =
                action === "pack"
                  ? undefined
                  : Math.min(baseBetAmount, playerChips);
        ...
        <p
          aria-live="polite"
          className="mt-2.5 flex items-center justify-center gap-2 text-xs text-[#A8B5AF]"
        >
          <span>
            Current bet{" "}
            <AnimatedChipCount
              value={currentBet}
              prefix="◉ "
              className="text-xs text-[#C9D3CE]"
            />
          </span>
          <span aria-hidden="true">·</span>
          <span className={isBlind ? "text-[#E8B04A]" : "text-[#8FC7A5]"}>
            {isBlind ? "Playing blind" : "Cards seen"}
          </span>
        </p>
      </div>
    </motion.section>
  ...
  return (
    <div className="flex items-center gap-2 rounded-xl border border-white/10 bg-[#07110E]/90 p-2 backdrop-blur-sm">
      ...
      <PressableButton
        ...
        ariaLabel={`Play chaal for ${formatChips(betAmount)}`}
      >
        Chaal{" "}
        <AnimatedChipCount
          value={betAmount}
      ...
      <PressableButton
        ...
        disabled={disabled}
        variant="secondary"
        className="rounded-lg p-2 text-[#E8B04A]"
        ariaLabel="Raise bet"
      >
