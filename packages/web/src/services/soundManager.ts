...
class SoundManager {
  ...
  private loadSettings() {
    try {
      const settings = JSON.parse(
        localStorage.getItem("teen-patti-ui") || "{}",
      );
  ...
  preload() {
    ...
    this.init();
    this.currentMusic = this.generateTone(130.81, 4, "sine", 0.035);
    this.currentMusic.loop(true);
    this.currentMusic.volume(this.musicVolume);
    if (this.musicEnabled) this.currentMusic.play();
    ...
    this.sounds.set(
      "button_click",
      this.generateTone(1000, 0.05, "square", 0.1),
    );
    ...
    this.sounds.set(
      "timer_urgent",
      this.generateTone(880, 0.15, "square", 0.4),
    );
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      notes.forEach((freq, idx) => {
        ...
        if (t >= noteStart) {
          ...
          sample +=
            Math.sin(2 * Math.PI * freq * noteT) * Math.exp(-noteT * 3) * 0.2;
        }
      });
    ...
    for (let i = 0; i < numSamples; i++) {
      ...
      notes.forEach((freq, idx) => {
        ...
        if (t >= noteStart) {
          const noteT = t - noteStart;
          sample +=
            Math.sin(2 * Math.PI * freq * noteT) * Math.exp(-noteT * 2) * 0.15;
          // Add harmony
          sample +=
            Math.sin(2 * Math.PI * freq * 1.5 * noteT) *
            Math.exp(-noteT * 3) *
            0.05;
        }
      });
  ...
  setMusicEnabled(enabled: boolean) {
    ...
    } else if (enabled && this.currentMusic && !this.currentMusic.playing()) {
