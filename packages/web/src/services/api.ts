...
const API_URL =
  configuredApiUrl &&
  (!import.meta.env.PROD || configuredApiUrl.startsWith("https://"))
    ? configuredApiUrl
    : import.meta.env.DEV
...
class ApiClient {
  ...
  loadTokens() {
    try {
      ...
      // Keep volatile credentials when browser storage is unavailable.
    }
  }
  ...
  private async request<T>(
    ...
    if (!API_URL)
      throw new Error("The production game server is not configured");
  ...
  private async performRefresh(): Promise<boolean> {
    const before = this.refreshToken;
    const refresh = async () => {
      this.loadTokens();
      if (this.refreshToken && this.refreshToken !== before) return true;
      return this.rotateToken();
    };
    return navigator.locks
      ? navigator.locks.request("teen-patti-refresh", refresh)
      : refresh();
  }
  private async rotateToken(): Promise<boolean> {
    try {
  ...
  async guestLogin(username?: string, referral?: ReferralAttribution | null) {
    const data = await this.request<{
      ...
      token: string;
      refreshToken: string;
      referralAttribution?: { attributed: boolean; reason?: string };
    }>("/auth/guest", {
      method: "POST",
      body: JSON.stringify({
        username,
  ...
  async upgradeAccount(data: {
    ...
    const result = await this.request<{
      ...
      token: string;
      refreshToken: string;
    }>("/auth/upgrade", { method: "POST", body: JSON.stringify(data) });
  ...
  async claimDailyReward() {
    return this.request<{ reward: any }>("/users/daily-reward", {
      method: "POST",
    });
  }
  ...
  async recordReferralShare(
    platform: ReferralSharePlatform,
    campaign = "table_circle",
  ) {
    return this.request<{ share: { id: string; createdAt: string } }>(
      "/referrals/share",
      {
        method: "POST",
  ...
  async equipReward(
    itemId: string,
    equip = true,
  ): Promise<{ equippedItems: Record<string, string> }> {
    return this.request("/referrals/equip", {
      method: "POST",
  ...
  async redeemBeli(itemId: string) {
    return this.request<{ beliBalance: number; entitlement: unknown }>(
      "/referrals/redeem",
      {
        method: "POST",
  ...
  async createPaymentOrder(packageId: string) {
    return this.request<{ orderId: string; amount: number; keyId: string }>(
  ...
  async verifyPayment(paymentId: string, orderId: string, signature: string) {
    return this.request<{ success: boolean; chips: string; balance: string }>(
      "/payments/verify",
      {
        method: "POST",
