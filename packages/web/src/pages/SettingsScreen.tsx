  ...
  const handleDeleteAccount = async () => {
    ...
    try {
      await api.deleteAccount(
        confirmation,
        user?.isGuest ? undefined : password,
      );
      logout();
      onNavigate("login");
    } catch (error) {
      ...
      <div className="flex h-full flex-col bg-[var(--color-background)] text-[var(--color-text)]">
        <header className="px-5 pb-4 pt-5">
          ...
          <p className="mt-1 text-sm text-[#7E8D85]">
            Sound, comfort, account, and responsible-play controls.
          </p>
        </header>

        <main className="flex-1 space-y-4 overflow-y-auto px-5 pb-28">
          <label className="block rounded-xl border border-white/10 p-4">
            Language
            <select
              aria-label="Language"
              value={locale}
              onChange={(e) => void setLocale(e.target.value as Locale)}
              className="ml-3 min-h-11 rounded-lg bg-[#193126] p-2"
            >
          ...
          <section className="overflow-hidden rounded-[22px] border border-white/10 bg-[#0E1B17]">
            ...
            >
              ...
              <span className="min-w-0 flex-1">
                ...
                <span className="block truncate text-xs text-[#66736D]">
                  {user?.username || "Guest"}
                </span>
              </span>
          ...
          <section
            aria-label="Play preferences"
            className="overflow-hidden rounded-[22px] border border-white/10 bg-[#0E1B17]"
          >
            <PreferenceRow
              icon={Volume2}
              label="Sound effects"
          ...
          <section className="overflow-hidden rounded-[22px] border border-white/10 bg-[#0E1B17]">
            <a
              ...
              className="flex min-h-14 items-center gap-3 border-b border-white/10 px-4 text-[var(--color-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#E8B04A]"
            >
            ...
            <a
              ...
              className="flex min-h-14 items-center gap-3 px-4 text-[var(--color-text)] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-[#E8B04A]"
            >
            ...
            {showDelete && (
              <div className="mt-4 space-y-3 border-t border-red-500/15 pt-4">
                <p className="text-sm leading-6 text-[#A99B94]">
                  Your profile and social content will be removed. Auditable
                  game, referral, and transaction records are retained without
                  your profile identifiers.
                </p>
                <label className="block text-xs font-semibold uppercase tracking-wider text-white/50">
                  ...
                  <input
                    ...
                    autoComplete="off"
                    className="mt-2 min-h-12 w-full rounded-xl border border-white/10 bg-[var(--color-background)] px-3 text-base normal-case tracking-normal text-[var(--color-text)] outline-none focus:border-[#B74035] focus:ring-2 focus:ring-[#B74035]/20"
                  />
                </label>
                {!user?.isGuest && (
                  <label className="block text-xs font-semibold uppercase tracking-wider text-white/50">
                ...
                {deleteError && (
                  <p role="alert" className="text-sm text-[#F2B1A9]">
                    {deleteError}
                  </p>
                )}
                <button
          ...
          <p className="text-center text-xs text-[#58645E]">
            Teen Patti Social · Adults 18+ · Social play only
          </p>
        </main>
  ...
  return (
    <div className="flex min-h-16 items-center gap-3 border-b border-white/10 px-4 last:border-b-0">
      ...
      <span className="flex-1">
        ...
        {description && (
          <span className="block text-xs text-[#66736D]">{description}</span>
        )}
      </span>
      <button
        type="button"
      ...
      >
        <span
          className={`absolute left-1 top-1 h-5 w-5 rounded-full ${enabled ? "bg-[#171006]" : "bg-white"} transition-transform ${enabled ? "translate-x-5" : "translate-x-0"}`}
        />
      </button>
    </div>
  );
