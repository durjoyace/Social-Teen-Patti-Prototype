...
app.get("/ready", async (_req, res) => {
  try {
    ...
    if (!roomManager.isReady) throw new Error("Rooms are recovering");
...
app.use(
  "/api/auth",
  rateLimit({ windowMs: 60_000, max: 20, name: "auth" }),
  authRouter,
);
...
app.use(
  "/api/referrals",
  rateLimit({ windowMs: 60_000, max: 60, name: "referrals" }),
  referralsRouter,
);
...
export const roomManager = setupSocketHandlers(io);
...
async function start() {
  try {
    ...
    const metricsTimer = setInterval(
      () =>
        console.log(
          JSON.stringify({ type: "game_metrics", ...roomManager.stats }),
        ),
      60_000,
    );
    metricsTimer.unref();
