  ...
  .superRefine((value, context) => {
    ...
    if (!/^(postgres|postgresql):\/\//.test(value.DATABASE_URL)) {
      context.addIssue({
        ...
        path: ["DATABASE_URL"],
        message:
          "DATABASE_URL must be a PostgreSQL connection URL in production",
      });
    }
    ...
    if (
      new Set(requiredSecrets.map(([, secret]) => secret)).size !==
    ...
    if (
      value.PURCHASES_ENABLED &&
      (!value.RAZORPAY_KEY_ID || !value.RAZORPAY_KEY_SECRET)
    ) {
      context.addIssue({
        code: z.ZodIssueCode.custom,
