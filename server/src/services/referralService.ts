...
const cleanTrackingValue = (value?: string) =>
  value?.trim().slice(0, 64) || undefined;
...
export async function attributeReferral(
  ...
  if (!isValidReferralCode(code))
    return { attributed: false as const, reason: "invalid_code" as const };
  ...
  if (!inviter)
    return { attributed: false as const, reason: "unknown_code" as const };
  if (inviter.id === inviteeId)
    return { attributed: false as const, reason: "self_referral" as const };

  const alreadyAttributed = await prisma.referral.findUnique({
    where: { inviteeId },
    select: { id: true },
  });
  ...
  const [deviceUses, recentIpUses] = await Promise.all([
    deviceHash
      ? prisma.referral.count({ where: { inviterId: inviter.id, deviceHash } })
      : Promise.resolve(0),
    ipHash
      ? prisma.referral.count({
          where: {
            ...
            ipHash,
            attributedAt: { gte: new Date(Date.now() - 24 * 60 * 60 * 1000) },
          },
        })
      : Promise.resolve(0),
  ]);
...
export async function getReferralSummary(userId: string) {
  ...
  const owned = new Set(
    user.rewardEntitlements.map((entitlement) => entitlement.itemId),
  );
  const nextMilestone =
    REFERRAL_MILESTONES.find((milestone) => milestone.count > activatedCount) ??
    null;
  ...
  return {
    ...
    catalog: BELI_REWARD_CATALOG.map((item) => ({
      ...item,
...
export async function qualifyReferralForGame(
  ...
  if (participants.size < 2 || !participants.has(inviteeId)) {
    return {
      ...
      reason: "not_qualifying_multiplayer_game" as const,
    };
  }
  ...
  try {
    return await serializable(async (tx) => {
      ...
      if (!referral)
        return { rewarded: false as const, reason: "not_referred" as const };
      ...
      const automatedRiskReview = riskFlags.some((flag) =>
        ["duplicate_device", "ip_velocity"].includes(flag),
      );
      if (
        automatedRiskReview ||
        recentRewards >= MAX_REWARDED_REFERRALS_PER_DAY
      ) {
        const finalFlags = [
          ...riskFlags,
          ...(recentRewards >= MAX_REWARDED_REFERRALS_PER_DAY
    ...
    if (
      ...
      if (current?.status === "PENDING") throw error; // Keep the durable job pending after a rolled-back award.
...
export async function redeemBeliReward(userId: string, itemId: string) {
  ...
  try {
    return await serializable(async (tx) => {
      const existing = await tx.rewardEntitlement.findUnique({
      ...
      if (!user) throw new ReferralError("User not found", 404);
      if (user.beliBalance < item.costBeli)
        throw new ReferralError("Not enough Club Points", 409);
...
export async function equipReward(
  ...
  return serializable(async (tx) => {
    const owned = await tx.rewardEntitlement.findUnique({
      where: { userId_itemId: { userId, itemId } },
    });
    if (!owned)
      throw new ReferralError("Unlock this extra before equipping it", 403);
    const user = await tx.user.findUniqueOrThrow({ where: { id: userId } });
    const equipped = { ...(user.equippedItems as Record<string, string>) };
    if (equip) equipped[item.type] = itemId;
    else if (equipped[item.type] === itemId) delete equipped[item.type];
    await tx.user.update({
      where: { id: userId },
      data: { equippedItems: equipped },
    });
    return { equippedItems: equipped };
  });
}
