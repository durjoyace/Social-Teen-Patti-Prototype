...
export async function deleteAccount(userId: string, password?: string) {
  ...
  if (await prisma.tableReservation.findUnique({ where: { userId } }))
    throw new AccountDeletionError(
      "Leave your table and wait for the hand to settle before deleting your account",
      409,
    );
  await prisma.$transaction(async (tx) => {
    ...
    await tx.notification.updateMany({
      ...
      data: {
        body: "A player update is no longer available",
