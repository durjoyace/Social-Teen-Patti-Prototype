...
if (result.error) {
  console.error(
    "Unable to run the pnpm production audit:",
...
if (
  ...
  console.error(
    "The pnpm audit JSON schema was not recognized; failing closed.",
  );
  process.exit(2);
}
...
if (blocked.length > 0) {
  console.error(
    "The web beta dependency gate found advisories outside the held mobile-only graph:",
  );
...
if (mobileOnlyCount > 0) {
  ...
  console.log(
    "Web/shared pnpm runtime audit passed with no production advisories.",
  );
}
